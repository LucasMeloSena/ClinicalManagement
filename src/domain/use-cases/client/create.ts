...
export class CreateClient {
    ...
    async execute(client: CreateClientDto): Promise<Client> {
        const existingClients = await this.clientRepository.findAll({email: client.email, phone: client.phone, cpf: client.cpf})
        if (existingClients.length > 0 ) {
            throw new BadRequestException("Already exists a client with provided data.")
        } 
